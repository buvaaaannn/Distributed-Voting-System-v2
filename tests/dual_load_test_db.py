#!/usr/bin/env python3
"""
Simultaneous load test: 250k law votes + 250k election votes (Direct DB)
Tests system under dual concurrent load
"""
import psycopg2
import random
import time
...
def law_vote_test():
    """Test law voting - 250k votes"""
    print("\n" + "="*70)
    print(f"LAW VOTE TEST - {NUM_VOTES:,} VOTES FOR '{LAW_ID}'")
    print("="*70)
    ...
    cur.execute("DELETE FROM vote_audit WHERE law_id = %s AND vote_hash LIKE %s", (LAW_ID, 'lawtest_%'))
    cur.execute("UPDATE vote_results SET oui_count = 0, non_count = 0, total_votes = 0 WHERE law_id = %s", (LAW_ID,))
    conn.commit()

    print(f"\nGenerating {NUM_VOTES:,} law votes...")
    ...
    for batch_start in range(0, NUM_VOTES, batch_size):
        ...
        for i in range(batch_start, batch_end):
            ...
            votes_data.append((
                ...
                vote,
                'accepted',
                '{"source": "dual_load_test", "submitted_at": "' + datetime.now().isoformat() + '"}'
            ))

        # Bulk insert
        cur.executemany("""
            INSERT INTO vote_audit (vote_hash, law_id, vote, status, metadata)
            VALUES (%s, %s, %s, %s, %s)
        """, votes_data)
        ...
        if (batch_end) % 25000 == 0 or batch_end == NUM_VOTES:
            ...
            print(f"   LAW: {batch_end:,}/{NUM_VOTES:,} votes ({rate:.0f} votes/sec)")

    # Update vote_results
    cur.execute("""
        UPDATE vote_results SET
            oui_count = (SELECT COUNT(*) FROM vote_audit WHERE law_id = %s AND vote = 'oui' AND vote_hash LIKE 'lawtest_%%'),
            non_count = (SELECT COUNT(*) FROM vote_audit WHERE law_id = %s AND vote = 'non' AND vote_hash LIKE 'lawtest_%%'),
            total_votes = (SELECT COUNT(*) FROM vote_audit WHERE law_id = %s AND vote_hash LIKE 'lawtest_%%')
        WHERE law_id = %s
...
def election_vote_test():
    """Test election voting - 250k votes"""
    print("\n" + "="*70)
    print(f"ELECTION VOTE TEST - {NUM_VOTES:,} VOTES")
    print("="*70)
    ...
    print(f"\nGenerating {NUM_VOTES:,} election votes...")
    start_time = time.time()
    batch_size = 5000
...
def main():
    ...
    print("\n" + "="*70)
    print("DUAL CONCURRENT LOAD TEST")
    ...
    print(f"\n✅ Both systems tested simultaneously!")
